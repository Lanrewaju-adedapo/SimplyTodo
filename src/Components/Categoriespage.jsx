...
const Categoriespage = ({ categoryId, open, activesection, onTaskClick, fetchCategoryTasks, Tasks, isLoading, Error }) => {
  ...
  return (
    <div className="flex flex-1 flex-col">
      <div className="px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
        <div className="flex items-center">
          <div className="flex items-center space-x-3 sm:space-x-4">
            ...
            <span className="rounded-full bg-gray-100 px-2 py-1 text-sm sm:text-lg lg:text-xl font-medium text-gray-600">{Tasks.length}</span>
          </div>
        </div>
      </div>
      ...
      {(Error !== 'Error fetching tasks:' && Tasks.length <= 0 && isLoading === false) &&
        <div className='flex h-screen w-full flex-col items-center justify-center gap-3 px-4'>
